...
format is unsuitable, because it takes tons of space and is very slow to work with in 
graphic editors. On the other hand, completely rasterizing the plot distorts important
text content and prevents readers from editing and copying. A solution here is to 
rasterize layers that have a lot of data points, keeping all the rest as vector.
...
The same function can be applied on the level of individual layers. It allows to rasterize
only some layers of the same type:
...
Mostly for legacy reasons, we have all popular geoms wrapped inside the package. However, we encourage users to use the `rasterise()` function instead.
...
## Box plots with jitter outliers

One type of plot with a potentially large number of small objects is geom_boxplot.
But with a large number of objects, outlier points can become noninformative:
...
```{r, fig.width=5, fig.height=5}
gg +
  geom_boxplot_jitter(outlier.size=0.1, outlier.jitter.width=0.3, outlier.alpha=0.5, raster.dpi=200)
```

This is a bit out of the main package topic, but still useful for visualization of large datasets.
