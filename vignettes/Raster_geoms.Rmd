...
## Raster points
### Points
Sometimes you need to publish a figure in a vector format. But in other cases, your figure contains thousands of points:
...
PrintFileSize <- function(gg, name) {
  invisible(ggsave(file.path(tempdir(), 'tmp.pdf'), gg, width = 4, height= 4))
  cat(name, ': ', file.info(file.path(tempdir(), 'tmp.pdf'))$size / 1024, ' Kb.\n', sep = '')
  unlink('tmp.pdf')
}
